...
def MakeFile(ReadingMethod):
    try:
        fvar.filepath_Main= fvar.folderpath_Result + "\\" + ReadingMethod + "\\main\\" + fvar.filename_PDF + ".txt"
        ...
        with open(fvar.filepath_Main, "a", encoding="UTF-8") as TextFile:
            # [>] Formata o arquivo e escreve um novo arquivo de saída
            TextFile.write(FormatTextFile(fvar.filepath_WithoutFormatting))
    except Exception as ExceptionError:
        error.Show("Ocorreu um erro desconhecido ao criar o arquivo com formatação 'main'.", ExceptionError)
